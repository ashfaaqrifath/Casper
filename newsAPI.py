...
def news_BBC(voice_var, log):
    ...
    casper_log(write=results, var=log)
...
def news_category(voice_var, log):
    ...
    if Headlines:
        ...
        casper_log(write=f"{articles['title'][-b+1:]}: {articles['title'][:-b-2]}.", var=log)
        casper_log(write=f"{articles['title'][-b+1:]} News: {articles['title'][:-b-2]}.", var=log)
    else:
