...
chronicle_log(write="CASPER - DESKTOP ASSISTANT v2.7.4\n" +
           str(date) + " \n<<< ACTIVITY LOG >>> \nBEGIN LOG >>>\n \n", var=stealth)
...
def command_engine(command): #The heart of Casper
    ...
    try:
        ...
        if "search" in command or "YouTube" in command or "google news" in command.lower():
            surf_web(command, voice_var=voice_engine, input_var=input_source, log=stealth)
            return
    ...
    # NewsAPI
            ...
            news_category(voice_var=voice_engine, input_var=input_source, log=stealth)
            return
        ...
        if "voice" in str(recognize):
            ...
            while True:
                ...
                if keyword.count(password) > 0:
                    ...
                    chronicle_log(Fore.RED + "<<< WRONG PASSWORD >>>".center(100), var=stealth)
...
# MIT License
# Copyright (c) 2023 Ashfaaq Rifath - Casper v2.7.4